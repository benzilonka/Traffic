...
def generate_routefile(vehicle_disterbution, vehicle_type, speed, duration):
    random.seed(speed)  # make tests reproducible
    ...
    with open("cross_1.rou.xml", "w") as routes:
        print >> routes, """<routes>
        ...
        for i in range(N):
            ...
            if random.uniform(0, 1) < pLL:
                print >> routes, '    <vehicle id="%i" type="typePassenger" route="left-left" depart="%i" />' % (
                vehNr, i)
                vehNr += 1
                lastVeh = i
            if random.uniform(0, 1) < pDL:
                print >> routes, '    <vehicle id="%i" type="typePassenger" route="down-left" depart="%i" />' % (
                vehNr, i)
                vehNr += 1
                lastVeh = i
            if random.uniform(0, 1) < pDR:
                print >> routes, '    <vehicle id="%i" type="typePassenger" route="down-right" depart="%i" />' % (
                vehNr, i)
                vehNr += 1
                lastVeh = i
            if random.uniform(0, 1) <= pDD:
                print >> routes, '    <vehicle id="%i" type="typePassenger" route="down-down" depart="%i" />' % (
                vehNr, i)
                vehNr += 1
                lastVeh = i
